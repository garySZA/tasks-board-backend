...
teamRouter.get('/creator/:id', [], getTeamsByCreatorId);
...
teamRouter.post(
    ...
    [
        ...
        check('nameTeam', 'El campo debe tener al menos 3 caracteres').isLength(
            { min: 3 }
        ),
        check('nameTeam', 'El campo debe tener máximo 50 caracteres').isLength({
            max: 50,
        }),
...
teamRouter.post(
    ...
    [
        ...
        check(
            'newUsers',
            'El campo debe ser una lista de ids válidos'
        ).isArray(),
        ...
        check('teamId', 'El campo debe ser un número positivo válido').isInt({
            min: 1,
        }),
        check('teamId').custom((value) =>
            existsElementByPKParam(value, 'team')
        ),
        validateFields,
    ],
    assignUsersToTeam
